...
public class Group : MonoBehaviour {
    ...
    void Update () {
        ...
        if (m_Gyro.rotationRate.y< -1) {
            tryChangePos(new Vector3(-1, 0, 0));
        } else if (m_Gyro.rotationRate.y>1) {  // Move right
            tryChangePos(new Vector3(1, 0, 0));
        ...
        } else if (m_Gyro.rotationRate.x < -1 || (Time.time - lastFall) >= (float)1 / Mathf.Sqrt(LevelManager.level)) {
            fallGroup();
